...
class SettingsManager {
  ...
  createSettingsUI() {
    ...
    if (!this.settingsBtn) {
      ...
      console.log('Settings button created and added to DOM.');
    } else {
  ...
  setupEventListeners() {
    ...
    if (btn) {
      ...
      console.warn('Settings button not found for event listener.');
    }
    ...
    if (closeBtn) {
      ...
      console.warn('Close settings button not found for event listener.');
    }
    ...
    if (colorOptions.length) {
      colorOptions.forEach(option => {
        option.addEventListener('click', (e) => {
          ...
          console.log('Color option selected:', color);
          this.selectColor(color);
        });
      });
    } else {
      console.warn('No color options found for event listeners.');
    }

    // Screen wake toggle
    const wakeToggle = document.getElementById('screen-wake-toggle');
    if (wakeToggle) {
      wakeToggle.addEventListener('change', (e) => {
        console.log('Screen wake toggle changed:', e.target.checked);
        this.toggleScreenWake(e.target.checked);
      });
    } else {
      console.warn('Screen wake toggle not found for event listener.');
    }

    // Reset used content button
    const resetBtn = document.getElementById('reset-used-content');
    if (resetBtn) {
      resetBtn.addEventListener('click', () => {
        console.log('Reset used content button clicked.');
        this.resetUsedContent();
      });
    } else {
      console.warn('Reset used content button not found for event listener.');
    }
