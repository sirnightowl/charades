...
class CardInfo {
  ...
  addInfoButtonToCard(cardElement, cardData) {
    ...
    const existingBtn = cardElement.querySelector('.info-btn');
    if (existingBtn) {
      existingBtn.remove();
    }
    ...
    // Store card data on the button element
    infoBtn.cardData = cardData;

    // Add event listener to show card info
    infoBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      this.showCardInfo(e.currentTarget.cardData);
    });
